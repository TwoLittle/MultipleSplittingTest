...
for l in range(nrep):
	for i in range(lenc):
		...
		for j in range(lenr):
			...
			#mpower[0,j] += (bsTest(X) < 0.05)
			#mpower[1,j] += (cqTest(X) < 0.05)
			#mpower[2,j] += (clxTest(X) < 0.05)
			#mpower[3,j] += (proj_test_ridge(X, 1) < 0.05)

			#for k in range(lensplt):
...
np.savetxt("pow_"+str(cov_type)+"_"+str(alt_type)+"_n"+str(n)+"_p"+str(p)+"_c"+str(intc)+"_"+str(jobid), mpower, fmt = "%.0f")
